...
import { desc, eq, gt, gte, lt, sql } from 'drizzle-orm'
...
export async function getAllSchedulesBySupportCenter({
  ...
  // Sub consulta para filtrar os alunos e disciplinas com base no supportCenterId
  ...
  // Sub consulta para filtrar horários de exames com base nos enrollments filtrados
...
export async function getPastSchedules({
  ...
  const schedules = await db
    ...
    .where(lt(filteredExamSchedules.scheduledDate, new Date()))
    .orderBy(desc(filteredExamSchedules.scheduledDate))
